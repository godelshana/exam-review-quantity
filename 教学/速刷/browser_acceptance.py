...
SESSION='quantity-isolation'
def bridge(action,args):
    with tempfile.NamedTemporaryFile('w',suffix='.json',encoding='utf-8',delete=False) as f:
        json.dump({'action':action,'args':args,'session':SESSION},f,ensure_ascii=False); name=f.name
    try:
        r=subprocess.run(['curl.exe','-sS','-X','POST','http://127.0.0.1:10086/command','-H','Content-Type: application/json','--data-binary','@'+name],capture_output=True,check=True)
...
def main():
    ...
    results['reload']=evaluate("""(()=>{const x=JSON.parse(localStorage.getItem('quantity-ladder-v3')),C=DrillCore;const pool=DATASET_VALIDATION.map(C.normalize);const out=C.sample(pool,{mode:'validation',size:1000,attempts:x.attempts,exposures:x.exposures});return JSON.stringify({persistedExposures:x.exposures.length,noExposedReentry:out.questions.every(q=>!x.exposures.includes(C.validationIdentity(q))),restoredAttempts:C.restore(x).db.attempts.length===x.attempts.length});})()""")
