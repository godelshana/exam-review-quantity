...
CODE=r"""(async()=>{
 ...
 set('source','train');set('level','all');set('mode','practice');set('length','5');set('topicFilter','all');
 for(const province of ['四川','广东','湖北']){
  set('provinceFilter',province);const count=Number($('filterInfo').textContent.match(/可选 (\d+)/)[1]);if(province==='湖北')assert(count>50,'cross-province reprints not lost');
 ...
 set('provinceFilter','all');set('catalogType','study');$('catalogPanel').open=true;$('catalogPanel').dispatchEvent(new Event('toggle'));assert($('catalogCount').textContent.startsWith(FULL_REAL_STUDY.length+' 题'),'every disputed/defective training item accessible');assert($('catalogItems').textContent.includes('仅研读'),'disputes not silent grading');assert(!$('catalogItems').querySelector('.catalog-practice'),'disputes cannot masquerade as verified questions');checks.push('non-scoring dispute studies');
 ...
 const bad=[];let cursor=0;await Promise.all(Array.from({length:12},async()=>{while(cursor<paths.length){const path=paths[cursor++];try{const img=new Image();img.loading='eager';img.style.cssText='position:fixed;left:0;top:0;width:1px;height:1px;opacity:0';const response=await fetch(path,{cache:'reload'});if(!response.ok)throw Error('HTTP '+response.status);const blobURL=URL.createObjectURL(await response.blob());img.src=blobURL;document.body.appendChild(img);try{await Promise.race([new Promise((resolve,reject)=>{img.onload=()=>resolve();img.onerror=()=>reject(Error('image load error'));}),new Promise((_,reject)=>setTimeout(()=>reject(Error('load timeout')),5000))]);if(!img.naturalWidth||!img.naturalHeight)bad.push(path);}finally{img.remove();URL.revokeObjectURL(blobURL);}}catch{bad.push(path);}}}));assert(bad.length===0,'all published formulas/figures decode: '+bad.join(','));checks.push(paths.length+' distinct image references decoded');
