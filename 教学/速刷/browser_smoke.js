...
const raw=[...window.HAND_LADDER_100,...window.CHALLENGE_Q,...window.DATASET_TRAIN,...window.DATASET_VALIDATION];
...
const question=()=>all.find(q=>q.s===$('stem').textContent);
...
assert($('result').querySelectorAll('.solution-part').length===30,'six explanation sections per question');
for(const body of $('result').querySelectorAll('.review-body')){const parts=body.querySelectorAll('.solution-part');assert(parts[1].textContent.includes('快法评级：'),'fast-value rating visible');assert(!/^(do|conditional|skip)$/.test(parts[4].querySelector('div').textContent.trim()),'decision translated into Chinese');if(parts[1].textContent.includes('快法评级：低'))assert(!parts[1].querySelector('strong').textContent.includes('最快'),'low-rated path not advertised as fastest');}
...
const q=question();assert(q,'find displayed question');const opts=[...document.querySelectorAll('#opts button')],good=opts.findIndex(b=>b.textContent.slice(3)===q.o[q.a]);const bad=(good+1)%4;opts[bad].click();const r=records().at(-1);assert(r.pick===bad&&!r.correct&&r.hinted&&r.paused,'actual pick / hint / pause flags');finishSkips();assert($('result').querySelector('summary').textContent.includes('你选'+'ABCD'[bad]),'actual wrong option in review');$('back').click();checks.push('hint, pause, wrong-pick mapping');
