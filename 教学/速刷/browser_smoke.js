...
for(const body of $('result').querySelectorAll('.review-body')){const parts=[...body.querySelectorAll('.solution-part')].map(p=>p.querySelector('strong')?.textContent||'');assert(parts.some(t=>t.includes('方法一'))&&parts.some(t=>t.includes('方法二')),'two solution paths visible');assert(!parts.some(t=>t.includes('normalPath')||t.includes('fastBoundary')),'no raw field names leaked');}
...
set('mode','practice');set('level','综合');$('start').click();const before=records().length;$('pause').click();document.querySelector('#opts button').click();assert(records().length===before,'paused answer blocked');$('pause').click();const hinted=$('hint').hidden?false:($('hint').click(),true);if(hinted)assert(!$('hintText').hidden,'training hint shows when available');
const q=question();assert(q,'find displayed question');const opts=[...document.querySelectorAll('#opts button')],good=q.sourceType==='real'?q.a:opts.findIndex(b=>b.textContent.slice(3)===q.o[q.a]);const bad=(good+1)%4;opts[bad].click();const r=records().at(-1);assert(r.pick===bad&&!r.correct&&(!hinted||r.hinted)&&r.paused,'actual pick / hint / pause flags');finishSkips();assert($('result').querySelector('summary').textContent.includes('你选'+'ABCD'[bad]),'actual wrong option in review');$('back').click();checks.push('hint, pause, wrong-pick mapping');
...
if(!$('start').disabled){
 ...
 set('source','validation-review');set('validationTier','V1');set('mode','practice');$('start').click();assert(!$('quiz').hidden,'seen-only review available');assert(!$('hint').disabled&&!$('pause').disabled,'review assistance allowed');if(!$('hint').hidden)$('hint').click();$('skip').click();assert(records().at(-1).firstValidation===false,'review not first');assert(JSON.stringify(C.transferStats(records(),all).firstValidation)===firstStats,'review does not backfill first score');exit();checks.push('V1 first exposure / restore / no preview / review contamination');
...
assert($('metrics').textContent.includes('训练'),'metrics rendered');
...
checks.push('two-path explanations / summary touch targets');
