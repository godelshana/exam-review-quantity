...
function isValidation(q){
 ...
 if([q.year,q.sourceYear,q.originYear].some(y=>[2024,2025,2026].includes(Number(y))))return true;
 return /(?:2024|2025|2026)/.test([q.uid,q.sourceQuestionUid,q.sourceLabel,q.provenance].filter(Boolean).join(' '));
}
...
function sample(source,{size=5,strategy='random',memory={},rng=Math.random,mode='train',attempts=[],exposures=[]}={}){
 ...
 const validationStems=new Set(source.filter(isValidation).map(q=>norm(q.s).replace(/\d+(\.\d+)?/g,'#')));
 let pool=source.filter(q=>{
  ...
  }else if(!['train','transfer'].includes(mode)||!trainEligible(q)||validationStems.has(k.replace(/\d+(\.\d+)?/g,'#')))return false;
...
return {levels,limits,normalize,eligible,isValidation,trainEligible,validationEligible,validationIdentity,sample,shuffled,summary,promotion,trainingMemory,transferStats,mappedExplanation,contentKey,currentAttempts,validAttempt,restore};
});